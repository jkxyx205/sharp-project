(function($) {
    ...
    Dialog.prototype = {
        ...
        tpl: '<div class="modal fade" id="{{id}}" tabindex="-1" role="dialog" aria-labelledby="modalCenterTitle" aria-hidden="true">\n' +
            ...
            '                <button class="btn btn-secondary" type="button" data-bs-dismiss="modal"><i class="fa fa-remove"></i> 关闭</button>\n' +
            '            </div>\n' +
            '        </div>\n' +
            '    </div>\n' +
            '</div>',
        ...
        _show: function() {
            new bootstrap.Modal(this.$modal[0], {backdrop: 'static'}).show()
        },
        _bindDom: function () {
            if (!this.domBind) {
