...
//        return namedParameterJdbcTemplate.getJdbcTemplate().queryForObject("SELECT "+selectColumn+" FROM "+ tableName +" WHERE id = ?", new BeanPropertyRowMapper<>(clazz), id);
    ...
    public <S> List<S> selectByIds(Collection<ID> ids, String columnName, Class<S> clazz){
        ...
        Assert.hasText(columnName, "columnName cannot be null");
        List<S> values = select(clazz, columnName, tableMeta.getIdMeta().getIdColumnName() + " IN (:ids)", Maps.of("ids", ids));
        return values;
    }
    @Override
    ...
    public Collection<T> insertWithoutCascade(Collection<T> entityList) {
        if (CollectionUtils.isNotEmpty(entityList)) {
            ...
            for (T entity : entityList) {
                Object[] params = tableMeta.getIdMeta().getId().strategy() == Id.GenerationType.IDENTITY ?  new Object[columnNameArray.length - 1] : new Object[columnNameArray.length];
                ...
                for (int i = 0; i < columnNameArray.length; i++) {
                    ...
                    if (Objects.equals(tableMeta.getIdMeta().getIdColumnName(), columnName)) {
                        if (tableMeta.getIdMeta().getId().strategy() == Id.GenerationType.IDENTITY) {
                            continue;
            ...
            if (tableMeta.getIdMeta().getId().strategy() == Id.GenerationType.IDENTITY) {
                ...
                String withoutIdColumn = Arrays.stream(tableMeta.getColumnNameArray())
                        .filter(s -> !tableMeta.getIdMeta().getIdColumnName().equals(s))
    ...
    public Collection<T> updateWithoutCascade(Collection<T> entityList) {
        if (CollectionUtils.isNotEmpty(entityList)) {
            ...
            tableDAO.batchUpdate(tableMeta.getTableName(),
                    ...
                    tableMeta.getIdMeta().getIdColumnName() + " = ?",
                    paramsList);
        }
                    ...
                    if (Objects.nonNull(reference.getManyToMany()) || Objects.nonNull(reference.getOneToMany())) {
                        ...
                        if (Objects.nonNull(reference.getManyToMany())) {
                            ...
                            if (CollectionUtils.isNotEmpty(list)) {
                                ...
                                for (Object referenceEntity : list) {
                                    tableDAO.insert(reference.getManyToMany().tableName(), reference.getManyToMany().joinColumnId() + "," + reference.getManyToMany().inverseJoinColumnId(), new HashMap<>(Maps.of(reference.getManyToMany().joinColumnId(), getIdValue(entity), reference.getManyToMany().inverseJoinColumnId(), getIdValue(referenceEntity))));
                                }
                            }
