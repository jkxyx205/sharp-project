...
public class SqliteTest {
    ...
    public void testInset() {
        if ("sqlite".equals(sharpDatabaseProperties.getType())) {
    ...
    public void testUpdate() {
        if ("sqlite".equals(sharpDatabaseProperties.getType())) {
            PgUser pgUser = PgUser.builder().id(893253198616248320L).age((short) 40).name("Tom").build();
            pgUser.setPgUserItemList(Arrays.asList(
                    PgUserItem.builder().id(893253198721105920L).name("Item-11").build(),
    ...
    public void testUpdate2() {
        if ("sqlite".equals(sharpDatabaseProperties.getType())) {
            PgUser pgUser = PgUser.builder().id(893253198616248320L).age((short) 99).name("Tom").build();
            pgUser.setPgUserItemList(Arrays.asList(
                    PgUserItem.builder().id(893253198721105920L).name("Item-11").build(),
    ...
    public void testQuery() {
        if ("sqlite".equals(sharpDatabaseProperties.getType())) {
            Optional<PgUser> optional = pgUserDAO.selectById(893253198616248320L);
            PgUser pgUser = optional.get();
            System.out.println(pgUser.getName());

            PgUserItem pgUserItem = pgUserItemDAO.selectById(893253198721105920L).get();
