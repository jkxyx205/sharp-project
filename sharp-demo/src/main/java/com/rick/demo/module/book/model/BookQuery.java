...
public class BookQuery {
    ...
    @Select(value = "select * from t_person where id = :personId", params = "personId@personId")
    Person person;

    /**
     * 标签
     */
    @Select(value = "select t_tag.* from t_book, t_tag, t_book_tag where t_book_tag.book_id = t_book.id AND t_book_tag.tag_id = t_tag.id AND t_book.id = :id", params = "id@id")
    List<Tag> tagList;
