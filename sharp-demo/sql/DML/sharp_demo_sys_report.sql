INSERT INTO `sharp-demo`.sys_report (id, pageable, sidx, sord, name, query_sql, summary_column_names, report_column_list, query_field_list, created_by, created_at, updated_by, updated_at, is_deleted) VALUES (619541501440958464, true, 'title', 'ASC', '图书报表', 'SELECT t_book.id, t_book.title, t_person.name, sys_dict.label "sexLabel"
FROM t_book,
...
WHERE t_book.person_id = t_person.id
  ...
  AND t_person.sex = :sex', null, '[{"name":"title","label":"书名","sortable":true,"align":"left","tooltip":false},{"name":"name","label":"作者","sortable":true,"align":"left","tooltip":false},{"name":"sexLabel","label":"性别","sortable":false,"align":"left","tooltip":false}]', '[{"name":"title","label":"书名","type":"TEXT"},{"name":"name","label":"作者","type":"TEXT"},{"name":"sex","label":"性别","type":"SELECT","extra_data":"sex"}]', 0, '2022-11-03 12:49:42', 0, '2023-03-16 14:37:54', false);
INSERT INTO `sharp-demo`.sys_report (id, pageable, sidx, sord, name, query_sql, summary_column_names, report_column_list, query_field_list, created_by, created_at, updated_by, updated_at, is_deleted) VALUES (668074769748373504, true, 'name', 'ASC', '人员信息表', 'SELECT t_user.name              AS "name",
